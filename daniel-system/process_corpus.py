...
def prepare_infos(infos, options):
    infos["document_path"] = check_abs_path(
        infos["document_path"], options.corpus)
    attr = ["isnot_clean", "ratio", "verbose",
            "debug", "name_out", "showrelevant"]
    for name in attr:
...
def check_abs_path(doc_path, corpus_path):
    if os.path.abspath(corpus_path):
        doc_path = os.path.dirname(
            os.path.abspath(corpus_path)) + "/" + doc_path
    return doc_path
...
def start_detection(options):
    ...
    if options.language != "all":
        corpus_to_process = {x: infos for x, infos in corpus_to_process.items(
        ) if infos["language"] == options.language}
    ...
    for id_file, infos in corpus_to_process.items():
        ...
        output_dic[id_file]["isnot_clean"] = str(
            output_dic[id_file]["isnot_clean"])
...
if __name__ == "__main__":
    ...
    print("\n%s docs proc. in %s seconds" %
          (str(cpt_doc), str(round(end-start, 4))))
