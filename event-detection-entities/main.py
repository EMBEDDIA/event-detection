...
parser.add_argument('--pos_embed', type=str, default='fix')
...
def load_data(paths, load_embed=True):
    ...
    if load_embed:
        if args.language != 'french':
            ...
            embed_doc = BertEmbedding(data.get_vocab('words'), model_dir_or_name='bert-base-multilingual-uncased',
                        pool_method='last', requires_grad=True, #layers='0,-1,-2,-3,-4,-5', 
                        include_cls_sep=False, dropout=0.5, auto_truncate=True,
                      word_dropout=0.01)
            ...
            embed = CamembertEmbedding(vocab=data.get_vocab(
                ...
                auto_truncate=True)
            embed_doc = CamembertEmbedding(vocab=data.get_vocab(
                'doc'), model_dir_or_name=args.pre_trained_model, requires_grad=True, layers='0',
                auto_truncate=True)

        return data, embed, embed_doc
    return data
