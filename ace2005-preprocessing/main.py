...
#        if token['characterOffsetBegin'] <= start_pos:
    ...
    assert start_idx != -1, "start_idx: {}, start_pos: {}, phrase: {}, tokens: {}".format(
        start_idx, start_pos, phrase, tokens)
    chars = ''
...
#        chars += remove_punc(tokens[start_idx + i]['originalText'])
        chars += remove_punc(tokens[start_idx + i].text)
        if remove_punc(phrase) in chars:
            end_idx = start_idx + i + 1
            break
          
    if end_idx == -1:
        print("end_idx: {}, end_pos: {}, phrase: {}, tokens: {}, chars:{}".format(
          end_idx, end_pos, phrase, tokens, chars))
        return None, None
#    assert end_idx != -1, "end_idx: {}, end_pos: {}, phrase: {}, tokens: {}, chars:{}".format(
#        end_idx, end_pos, phrase, tokens, chars)
    ...
    for item in data:
        ...
        for entity_mention in item['golden-entity-mentions']:
            if check_diff(''.join(words[entity_mention['start']:entity_mention['end']]), entity_mention['text'].replace(' ', '')):
                ...
                print(
                    'Actual:', words[entity_mention['start']:entity_mention['end']])
                print('start: {}, end: {}, words: {}'.format(
                    entity_mention['start'], entity_mention['end'], words))
        ...
        for event_mention in item['golden-event-mentions']:
            ...
            if check_diff(''.join(words[trigger['start']:trigger['end']]), trigger['text'].replace(' ', '')):
                ...
                print('start: {}, end: {}, words: {}'.format(
                    trigger['start'], trigger['end'], words))
            for argument in event_mention['arguments']:
                if check_diff(''.join(words[argument['start']:argument['end']]), argument['text'].replace(' ', '')):
                    ...
                    print('start: {}, end: {}, words: {}'.format(
                        argument['start'], argument['end'], words))
...
def preprocessing(data_type, files):
    ...
    for file in tqdm(files):
        ...
        for item in parser.get_data():
            data = dict()
            data['doc_id'] = parser.doc_id
            data['sentence'] = item['sentence']
            data['golden-entity-mentions'] = []
            data['golden-event-mentions'] = []

#            try:
#                nlp_res_raw = nlp.annotate(item['sentence'], properties={
#                                           'annotators': 'tokenize,ssplit,pos,lemma,parse'})
#                nlp_res = json.loads(nlp_res_raw)
#            except Exception:
#                print('[Warning] StanfordCore Exception: ',
#                      nlp_res_raw, 'This sentence will be ignored.')
...
#                continue
            ...
            tokens = nlp_spacy(item['sentence'])
#            from nltk.tokenize import word_tokenize
#            from nltk.tokenize import sent_tokenize
#            sentences = sent_tokenize(item['sentence'])
...
#                data['stanford-colcc'].append('{}/dep={}/gov={}'.format(
#                    dep['dep'], dep['dependent'] - 1, dep['governor'] - 1))

            data['words'] = [token.text for token in tokens]
...
#            data['parse'] = nlp_res['sentences'][0]['parse']
            ...
            for entity_mention in item['golden-entity-mentions']:
                ...
                if start_idx == None:
                    print(item['sentence'], '/=', entity_mention['text'])
                    continue
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', help="Path of ACE2005 English data",
                        default='./data/ace_2005_td_v7/data/English')
