...
def clean_text(text):
    doc = nlp(text)
    tokens = list([x for x in doc])# if x.text not in english_stopwords and (x.text not in punctuation)])
    return tokens
...
def prepare_dataset(file, 
    ...
    data_pkl_path = os.path.join(output_dir, "data_" + type_data + '_' + embeddings_type + ".pkl.gz")
    if generate_data == True:
        ...
        df = pd.read_csv(file, sep='\t', names=['doc_id',  'type', 'subtype', 'old_index',
                         ...
                         converters={"anchors": literal_eval}).sample(frac=.5) #TODO
    ...
    #            entities = get_entities(sentence)
                ...
                if len(clean_tokens) < fixed_size:
                    ...
                    for idx in range(len(clean_tokens), fixed_size):
                        previous_token_ids[idx] = getWordIdx('PADDING', word2Idx)
                else:
                            ...
                            if target_word in anchors_indexes:
                                if type(anchors_indexes[target_word]) == list:
                                    if len(event_ids[target_word]) > 0:
                                        ...
                                        details_event[1] = str(event_ids[target_word][0])
                                        details_event[2] = str(anchors_indexes[target_word][0])
...
#                                                what += ' '
                                                ...
                                                if window[idx] not in ['PADDING', 'UNKNOWN']:
                                                    try:
                                                        all_char[i] = getCharIdx(char, char2Idx)
#                                                        what += char
                                                    except:
                                                            import pdb;pdb.set_trace()
...
#                                    words[token] = True
    ...
    print('Generating embeddings for unknown words. Known words: {} Unknown words: {}'.format(
    ...
    print(len(word2Idx))
    print('Saving processed data in', output_dir)
    word2IdxPklPath = os.path.join(output_dir, "word2Idx_"+args.embeddings+".pkl.gz")
    f = gzip.open(word2IdxPklPath, 'wb')
    pkl.dump(word2Idx, f, -1)
    f.close()

    idx2wordPklPath = os.path.join(output_dir, "idx2word_"+args.embeddings+".pkl.gz")
    f = gzip.open(idx2wordPklPath, 'wb')
    pkl.dump(idx2word, f, -1)
    f.close()

    char2idxPklPath = os.path.join(output_dir, "char2idx_"+args.embeddings+".pkl.gz")
    f = gzip.open(char2idxPklPath, 'wb')
    pkl.dump(char2Idx, f, -1)
    f.close()

    idx2charPklPath = os.path.join(output_dir, "idx2char_"+args.embeddings+".pkl.gz")
...
if __name__ == '__main__':
    ...
    embeddingsPklPath = os.path.join(args.output_directory, 'embeddings_' + args.embeddings + '.pkl.gz')
