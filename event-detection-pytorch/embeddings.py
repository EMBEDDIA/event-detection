...
def load_embedding(fname, format="word2vec_bin", normalize=True,
    ...
    assert format in ['word2vec_bin', 'word2vec',
                      'glove', 'dict'], "Unrecognized format"
...
def fetch_NMT(which="DE", normalize=True, lower=False, clean_words=False):
    ...
    fname = {"FR": "Trans_embds/D_RNN_500k_144h.pkl",
...
def fetch_conceptnet_numberbatch(clean_words=False):
    ...
    """
    path = _fetch_file(
        url='https://conceptnet.s3.amazonaws.com/downloads/2017/numberbatch/numberbatch-en-17.06.txt.gz',
        data_dir='embeddings',
        uncompress=False,
        verbose=1)
    return load_embedding(
